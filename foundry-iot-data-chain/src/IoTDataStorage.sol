...
contract IoTDdataStorage {
    error IoTDataStorage__NotOwner();
    error IoTDataStorage__DeviceAlredyRegistred();
    error IoTDataStorage__DeviceNotRegistered();
    error IoTDataStorage__NoMeasurements();

    struct Device { //rappresenta un dispositivo IoT
        bool isRegistered;
        string metadataURI; //stringa libera per info su dispositivo
        uint256 registredAt; //timestamp di registrazione, qui salveremo block.timestamp
    }

    struct Measurement { //rappresenta una misurazione
    ...
    event DeviceRegistred(address indexed deviceAddress, string metadataURI);
    ...
    function RegisterDevice(
        ...
        if (s_devices[deviceAddress].isRegistered) {
            revert IoTDataStorage__DeviceAlredyRegistred();
        }
        ...
        emit DeviceRegistred(deviceAddress, metadataURI);
    }

    function recordMeasurement(
        int256 value
    ) external onlyRegisteredDevice(msg.sender) {
        // per ora mettiamo solo una valore nuemrico. poi potremmo aggiungere di verse grandezze misurate

        s_measurements[msg.sender].push(
            Measurement({value: value, timestamp: block.timestamp})
        );
