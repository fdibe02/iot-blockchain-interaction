...
// File generato automaticamente da scripts/deploy-sepolia.sh
// Non modificare a mano.

var contractAddress = "$CONTRACT_ADDRESS";
EOF

if [ ! -f "$SEPOLIA_ENV_FILE" ]; then
  echo "Errore: file env Sepolia non trovato:"
  echo "$SEPOLIA_ENV_FILE"
  exit 1
fi
...
" "$SEPOLIA_ENV_FILE" "$CONTRACT_ADDRESS" "$ENV_STORAGE_MODE"
